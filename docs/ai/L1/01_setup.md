...
- **pip** + `venv` for the canonical Python workflow, or **uv** with the root `pyproject.toml`.
...
`setup:env` copies `server/.env.example` → `server/.env.local` if missing. `setup:backend` selects Python 3.10+ (or `PYTHON_BIN`), recreates `server/venv`, upgrades pip, and installs `requirements.txt` through `venv/bin/python3`. `setup:frontend` runs `bun install`. `setup:deps` exists for `bun run dev:check`, not for `bun run setup`.

The optional uv workflow runs from the repository root:
...
`pyproject.toml` and `server/requirements.txt` pin `agora-agents` to the same
...
| `GOOGLE_API_KEY`        | Python (server)      | Yes      | Gemini provider API key; server only.                                  |
