...
bun.lock
pyproject.toml            # uv metadata; maps agora-agents to the sibling preview SDK
...
- **Two supported Python installers** — root `pyproject.toml` supports uv; `server/requirements.txt` supports the canonical `server/venv` + pip workflow.
