...
- **Greeting:** edit `DEMO_GREETING` in `server/src/agent.py`.
- **Model selection/voice:** edit `DEMO_MODELS` and `GeminiLive(...)` arguments. The pre-call UI and start request use the Gemini 3.8 IDs; Extended Thinking exposes one of the SDK's thinking levels.
- **VAD:** edit `turn_detection` on the MLLM provider.
...
- **Backend (FastAPI):** install deps from `server/requirements.txt`, set `AGORA_APP_ID`, `AGORA_APP_CERTIFICATE`, `GOOGLE_API_KEY` (pass `PORT` only as a runtime override), and run `python3 server/src/server.py` or `uvicorn server.src.server:app --host 0.0.0.0 --port $PORT`.
