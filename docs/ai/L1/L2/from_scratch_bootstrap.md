...
4. Implement `server/src/agent.py` with an `Agent` class that reads env once, constructs `AsyncAgora`, builds `AgoraAgent` with a single `GeminiLive` voice provider, starts async sessions, stores sessions by `agent_id`, and stops by active session or `client.stop_agent`.
