...
extension_points:
  ...
  - id: agent.managed-config
    name: Gemini MLLM prompt, model, thinking level, VAD, and voice
...
- The Google API key stays server-side; the Gemini MLLM and its prompt stay in the FastAPI agent configuration.
...
- Gemini model IDs, thinking defaults, VAD timing, voice IDs, and prompt text; update docs and tests when changing them.
