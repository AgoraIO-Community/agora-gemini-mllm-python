...
def test_start_agent_calls_agent_and_returns_shape(client):
    ...
    assert client.fake_agent.selection == ("models/gemini-3.8-live", None)


def test_start_agent_forwards_model_selection(client):
    response = client.post("/startAgent", json={"channelName": "ch", "rtcUid": 111, "userUid": 222, "model": "models/gemini-3.8-live-extended-thinking", "thinkingLevel": "high"})
    assert response.status_code == 200
    assert client.fake_agent.selection == ("models/gemini-3.8-live-extended-thinking", "high")
