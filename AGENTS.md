...
- Agent config: the pre-call model picker chooses Gemini 3.8 Live or Gemini 3.8 Live Extended Thinking; only Extended Thinking receives the selected low/medium/high thinking level. FastAPI sends these public IDs directly to the preview gateway. GOOGLE_API_KEY is required.
...
- `bun run dev:parallel` starts FastAPI on `http://localhost:8110`; Next.js chooses an available frontend port.
