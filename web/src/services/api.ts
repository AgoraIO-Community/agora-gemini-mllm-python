...
export async function startAgent(channelName: string, rtcUid: number, userUid: number, options?: { model: 'models/gemini-3.8-live' | 'models/gemini-3.8-live-extended-thinking'; thinkingLevel?: 'low' | 'medium' | 'high' }): Promise<string> {
  const payload = { channelName, rtcUid, userUid, ...options }
